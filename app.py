...
def generate_recommendations_dice(input_data, prediction, dice_explainer, model):
    ...
    try:
        ...
        st.info("💡 DiCE encountered an error. Please check the error message above or contact support.")
...
with st.expander("🧪 Interactive Mix Optimizer", expanded=False):
    ...
    # Summary
    if difference > 0:
        st.success(f"✅ Modified mix shows improvement of **{difference:.2f} MPa** ({(difference/original_prediction)*100:.1f}%)")
    elif difference < 0:
        st.error(f"⚠️ Modified mix shows decrease of **{abs(difference):.2f} MPa** ({abs(difference/original_prediction)*100:.1f}%)")
    else:
        st.info("➡️ No change in predicted strength")
